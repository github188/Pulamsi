 ...
 *
 */
public class SearchGoodsWrapperGet  extends RecyclerView.OnScrollListener implements  PtrHandler {
  ...
  public SearchGoodsWrapperGet(Activity activity) {
    mActivity = activity;
  }

  public SearchGoodsWrapperGet setListView(TRecyclerView listView) {
    mListView = listView;
    return this;
  }

  public SearchGoodsWrapperGet setLayoutManager(StaggeredGridLayoutManager layoutManager) {
    mLayoutManager = layoutManager;
    return this;
  }

  public SearchGoodsWrapperGet setListAdapter(HaiBaseListAdapter adapter) {
    mListAdapter = adapter;
    return this;
  }
  public SearchGoodsWrapperGet setProgressWheel(ProgressWheel progressWheel) {
    this.progressWheel = progressWheel;
    return this;
  }

  public SearchGoodsWrapperGet setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
    mLoadMoreHandler = loadMoreHandler;
    mIsLoadMoreOpen = true;
    return this;
  }

  public SearchGoodsWrapperGet setPtrStylelFrameLayout(PtrStylelFrameLayout ptrStylelFrameLayout) {
    this.ptrStylelFrameLayout = ptrStylelFrameLayout;
    mIsPullToRefreshOpen = true;
    return this;
  }

  public SearchGoodsWrapperGet setRequestListener(RequestListener requestListener) {
    mRequestListener = requestListener;
    return this;
  }

  public SearchGoodsWrapperGet setOnScrollListener(OnScrollListener onScrollListener) {
  ...
  private void getData(String url, final Map<String, String> bizParams) {
    try {
      String URL1 = url + "?productName=" + URLEncoder.encode(mBizParams.get("productName"), "utf-8")+ "&pageNumber=" + mBizParams.get("pageNumber") + "&pageSize=10&searchProperty=&searchValue=";
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            ...
            SearchGoodsList temp = gson.fromJson(
                    result, SearchGoodsList.class);
            if (temp.getList() != null && temp.getList().size() < 10) {
              removeFooter();
            }
            if (mRequestListener != null){
              mRequestListener.onRequestSuccess(temp.getList());
            }
            // 下拉刷新时，需要清空数据
