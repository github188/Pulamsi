...
public class ImageUtil {
   ...
   * @return 改写优化过的图片url
   */
  public static String rewriteCDNUrl(String originalUrl, int displayWidth, int displayHeight, boolean requestOriginSize) {
    if (originalUrl == null) {
      return null;
    }
...
//    }
    ...
    suffixParams.quality =
        shouldLoadLowImageQuality() ? (NetWorkUtil.is2G() ? IMAGE_QUALITY_EXTRA_LOW : IMAGE_QUALITY_LOW) : IMAGE_QUALITY_HIGH;
   ...
   * @return cdn后缀
   */
  private static String getImageUrlCDNSuffix(CDNSuffixParams suffixParams) {
    ...
    if (!suffixParams.requestOriginSize) {
      ...
      if (suffixParams.displayHeight <= 0) {
        ...
        // 默认屏幕宽度
        if (cdnWidth <= 0) {
          cdnWidth = PulamsiApplication.ScreenWidth;
        }
    ...
    if (suffixParams.useWebp) {
      ...
      if (HaiLog.isPrintLog) {
        HaiLog.d("webp available:", Boolean.TRUE.toString());
      }
    }
...
//    }
    ...
    if (NetWorkUtil.isWifi() && !webpDynamicSwitch) {
      return false;
    }
  ...
  public static boolean shouldLoadLowImageQuality() {
    ...
    return ((PulamsiApplication.autoSwitchPic && !isWifi) || (!PulamsiApplication.autoSwitchPic && !PulamsiApplication.switch2high));
  }
  ...
  public static boolean isHighResolution() {
    return PulamsiApplication.ScreenWidth >= 640;
  }

  public static boolean isLowResolution() {
    return PulamsiApplication.ScreenWidth < 480;
  }
