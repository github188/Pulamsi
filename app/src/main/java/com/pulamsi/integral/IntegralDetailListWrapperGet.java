...
import com.pulamsi.integral.entity.IntegralDetailResult;
 ...
 */
public class IntegralDetailListWrapperGet extends RecyclerView.OnScrollListener implements PtrHandler {
     ...
     * 列表对应数据适配器
     */
    private HaiBaseListAdapter<IntegralDetail> mListAdapter;
    ...
    public IntegralDetailListWrapperGet(Activity activity) {
        mActivity = activity;
    }

    public IntegralDetailListWrapperGet setListView(TRecyclerView listView) {
        mListView = listView;
        return this;
    }

    public IntegralDetailListWrapperGet setLayoutManager(StaggeredGridLayoutManager layoutManager) {
        mLayoutManager = layoutManager;
        return this;
    }

    public IntegralDetailListWrapperGet setListAdapter(HaiBaseListAdapter adapter) {
        mListAdapter = adapter;
        return this;
    }

    public IntegralDetailListWrapperGet setProgressWheel(ProgressWheel progressWheel) {
        this.progressWheel = progressWheel;
        return this;
    }

    public IntegralDetailListWrapperGet setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
        mLoadMoreHandler = loadMoreHandler;
        mLoadMoreHandler.setLoadingMsg(R.string.integral_detail_loading_more_footer);
        mLoadMoreHandler.setLoadFinishMsg(R.string.integral_detail_load_finish_footer);
        mIsLoadMoreOpen = true;
        return this;
    }

    public IntegralDetailListWrapperGet setPtrStylelFrameLayout(PtrStylelFrameLayout ptrStylelFrameLayout) {
        this.ptrStylelFrameLayout = ptrStylelFrameLayout;
        mIsPullToRefreshOpen = true;
        return this;
    }

    public IntegralDetailListWrapperGet setRequestListener(RequestListener requestListener) {
        mRequestListener = requestListener;
        return this;
    }

    public IntegralDetailListWrapperGet setOnScrollListener(OnScrollListener onScrollListener) {
        ...
        StringRequest dataRequest = new StringRequest(Request.Method.GET,
            ...
            public void onResponse(String result) {
                if (result != null) {
                    try {
                        Gson gson = new Gson();
                        IntegralDetailResult integralDetailResult = gson.fromJson(result, IntegralDetailResult.class);
                        updateListData(integralDetailResult.getList());
            ...
            public void onResponse(String result) {
                if (result != null) {
                    try {
                        Gson gson = new Gson();
                        IntegralDetailResult integralDetailResult = gson.fromJson(result, IntegralDetailResult.class);
                            if (integralDetailResult.getList() != null && integralDetailResult.getList().size() < 20) {
                                removeFooter();
                            }
                            if (mRequestListener != null) {
                                mRequestListener.onRequestSuccess(integralDetailResult.getList());
                            }
                            // 下拉刷新时，需要清空数据
