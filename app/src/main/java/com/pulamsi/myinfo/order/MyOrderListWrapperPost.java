 ...
 */
public class MyOrderListWrapperPost extends RecyclerView.OnScrollListener implements PtrHandler {
    ...
    public MyOrderListWrapperPost(Activity activity) {
        mActivity = activity;
    }

    public MyOrderListWrapperPost setListView(TRecyclerView listView) {
        mListView = listView;
        return this;
    }

    public MyOrderListWrapperPost setLayoutManager(StaggeredGridLayoutManager layoutManager) {
        mLayoutManager = layoutManager;
        return this;
    }

    public MyOrderListWrapperPost setListAdapter(HaiBaseListAdapter adapter) {
        mListAdapter = adapter;
        return this;
    }

    public MyOrderListWrapperPost setLoadViewHelper(LoadViewHelper loadViewHelper) {
        this.loadViewHelper = loadViewHelper;
        return this;
    }

    public MyOrderListWrapperPost setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
        mLoadMoreHandler = loadMoreHandler;
        mLoadMoreHandler.setLoadingMsg(R.string.order_loading_more_footer);
        mLoadMoreHandler.setLoadFinishMsg(R.string.order_load_finish_footer);
        mIsLoadMoreOpen = true;
        return this;
    }

    public MyOrderListWrapperPost setPtrStylelFrameLayout(PtrStylelFrameLayout ptrStylelFrameLayout) {
        this.ptrStylelFrameLayout = ptrStylelFrameLayout;
        mIsPullToRefreshOpen = true;
        return this;
    }

    public MyOrderListWrapperPost setRequestListener(RequestListener requestListener) {
        mRequestListener = requestListener;
        return this;
    }

    public MyOrderListWrapperPost setOnScrollListener(OnScrollListener onScrollListener) {
    ...
    public void init() {
        ...
        mListView.setItemAnimator(null);//去除动画
    ...
    public void updateListData(List data) {
        if (data != null && data.size() > 0) {
            ...
            for (Order order : orderList) {
                if (order.getOrderType() == 2){// 0-普通定单；1-采购定单；2-总订单（拆单）
                    for (Order orderSet : order.getOrderSet()) {
                        orderDateList.add(orderSet);
                    }
                }else {
                    orderDateList.add(order);
                }
            }


            mListAdapter.addDataList(orderDateList);
...
//      mListAdapter.notifyDataSetChanged();
        ...
        StringRequest dataRequest = new StringRequest(Request.Method.POST,
            ...
            public void onResponse(String result) {
                if (result != null) {
                    try {
                        Gson gson = new Gson();
                        List<Order> lists = gson.fromJson(result, new TypeToken<List<Order>>() {
                        }.getType());
            ...
            public void onResponse(String result) {
                ...
                if (result != null) {
                    try {
                        ...
                        //应该放在清除数据之后
                        if (mRequestListener != null) {
                            mRequestListener.onRequestSuccess(lists);
                        }
                    } catch (Exception e) {
                        LogUtils.e("第一次请求数据出错");
                    }
                }
            }
        }, new Response.ErrorListener() {
    ...
    public void onRefreshBegin(PtrFrameLayout frame) {
        if (null == mBizParams || null == mListAdapter) {
            ToastUtil.showToast("mBizParams为NUll");
            return;
        }
        // 业务参数回到第一页
