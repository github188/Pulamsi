...
import com.pulamsi.myinfo.wallet.entity.MemberAccountTranCount;
 ...
 *
 */
public class BalanceDetailListWrapperPost extends RecyclerView.OnScrollListener implements  PtrHandler {
  ...
  /** 列表对应数据适配器 */
  private HaiBaseListAdapter<MemberAccountTran> mListAdapter;
  ...
  public BalanceDetailListWrapperPost(Activity activity) {
    mActivity = activity;
  }

  public BalanceDetailListWrapperPost setListView(TRecyclerView listView) {
    mListView = listView;
    return this;
  }

  public BalanceDetailListWrapperPost setLayoutManager(StaggeredGridLayoutManager layoutManager) {
    mLayoutManager = layoutManager;
    return this;
  }

  public BalanceDetailListWrapperPost setListAdapter(HaiBaseListAdapter adapter) {
    mListAdapter = adapter;
    return this;
  }
  public BalanceDetailListWrapperPost setProgressWheel(ProgressWheel progressWheel) {
    this.progressWheel = progressWheel;
    return this;
  }

  public BalanceDetailListWrapperPost setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
    mLoadMoreHandler = loadMoreHandler;
    mLoadMoreHandler.setLoadingMsg(R.string.my_info_wallet_loading_more_footer);
    mLoadMoreHandler.setLoadFinishMsg(R.string.my_info_wallet_load_finish_footer);
    mIsLoadMoreOpen = true;
    return this;
  }

  public BalanceDetailListWrapperPost setPtrStylelFrameLayout(PtrStylelFrameLayout ptrStylelFrameLayout) {
    this.ptrStylelFrameLayout = ptrStylelFrameLayout;
    mIsPullToRefreshOpen = true;
    return this;
  }

  public BalanceDetailListWrapperPost setRequestListener(RequestListener requestListener) {
    mRequestListener = requestListener;
    return this;
  }

  public BalanceDetailListWrapperPost setOnScrollListener(OnScrollListener onScrollListener) {
  ...
  private void getData(String url, final Map<String, String> bizParams) {
    StringRequest dataRequest = new StringRequest(Request.Method.POST,
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            Gson gson = new Gson();
            MemberAccountTranCount memberAccountTranCount = gson.fromJson(result, MemberAccountTranCount.class);
            updateListData(memberAccountTranCount.getAccountTranMobileList());
          } catch (Exception e) {
          }
        }
      }
    ...
    dataRequest.setRetryPolicy(new DefaultRetryPolicy(60 * 1000, 1, 1.0f));
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            Gson gson = new Gson();
            MemberAccountTranCount memberAccountTranCount = gson.fromJson(result, MemberAccountTranCount.class);
            if (memberAccountTranCount.getAccountTranMobileList() != null && memberAccountTranCount.getAccountTranMobileList().size() < 20) {
              removeFooter();
            }
            if (mRequestListener != null){
              mRequestListener.onRequestSuccess(memberAccountTranCount.getAccountTranMobileList());
            }
            // 下拉刷新时，需要清空数据
