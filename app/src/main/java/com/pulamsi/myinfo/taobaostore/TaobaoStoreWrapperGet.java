 ...
 *
 */
public class TaobaoStoreWrapperGet extends RecyclerView.OnScrollListener implements  PtrHandler {
  ...
  /** 列表对应数据适配器 */
  private HaiBaseListAdapter<ChannelMobile> mListAdapter;
  ...
  public TaobaoStoreWrapperGet(Activity activity) {
    mActivity = activity;
  }

  public TaobaoStoreWrapperGet setListView(TRecyclerView listView) {
    mListView = listView;
    return this;
  }

  public TaobaoStoreWrapperGet setLayoutManager(StaggeredGridLayoutManager layoutManager) {
    mLayoutManager = layoutManager;
    return this;
  }

  public TaobaoStoreWrapperGet setListAdapter(HaiBaseListAdapter adapter) {
    mListAdapter = adapter;
    return this;
  }
  public TaobaoStoreWrapperGet setProgressWheel(ProgressWheel progressWheel) {
    this.progressWheel = progressWheel;
    return this;
  }

  public TaobaoStoreWrapperGet setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
    mLoadMoreHandler = loadMoreHandler;
    mLoadMoreHandler.setLoadingMsg(R.string.my_info_taobaostore_loading_more_footer);
    mLoadMoreHandler.setLoadFinishMsg(R.string.my_info_taobaostore_load_finish_footer);
    mIsLoadMoreOpen = true;
    return this;
  }

  public TaobaoStoreWrapperGet setPtrStylelFrameLayout(PtrStylelFrameLayout ptrStylelFrameLayout) {
    this.ptrStylelFrameLayout = ptrStylelFrameLayout;
    mIsPullToRefreshOpen = true;
    return this;
  }

  public TaobaoStoreWrapperGet setRequestListener(RequestListener requestListener) {
    mRequestListener = requestListener;
    return this;
  }

  public TaobaoStoreWrapperGet setOnScrollListener(OnScrollListener onScrollListener) {
  ...
  private void getData() {
    String channelMgrImplurlPath = url+ "?pageNumber="+mBizParams.get(PAGE_PARAM)+"&pageSize=10";
    StringRequest channelstringRequest = new StringRequest(Request.Method.GET,
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            Gson gson = new Gson();
            ChannelMobileTotal channelMobileTotal = gson.fromJson(result, ChannelMobileTotal.class);
            updateListData(channelMobileTotal.getList());
          } catch (Exception e) {
    ...
    channelstringRequest.setRetryPolicy(new DefaultRetryPolicy(60 * 1000, 1, 1.0f));
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            Gson gson = new Gson();
            ChannelMobileTotal channelMobileTotal = gson.fromJson(result, ChannelMobileTotal.class);
            if (channelMobileTotal.getList() != null && channelMobileTotal.getList().size() < 10) {
              removeFooter();
            }
            if (mRequestListener != null){
              mRequestListener.onRequestSuccess(channelMobileTotal.getList());
            }
            // 下拉刷新时，需要清空数据
