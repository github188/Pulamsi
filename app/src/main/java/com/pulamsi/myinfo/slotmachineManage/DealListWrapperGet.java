...
public class DealListWrapperGet extends RecyclerView.OnScrollListener implements  PtrHandler {
  ...
  public DealListWrapperGet setListView(TRecyclerView listView) {
    mListView = listView;
    return this;
  }

  public DealListWrapperGet setLayoutManager(StaggeredGridLayoutManager layoutManager) {
    mLayoutManager = layoutManager;
    return this;
  }

  public DealListWrapperGet setListAdapter(HaiBaseListAdapter adapter) {
  ...
  public DealListWrapperGet setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
    mLoadMoreHandler = loadMoreHandler;
    mLoadMoreHandler.setLoadingMsg(R.string.slotmachine_manage_deallist_loading_more_footer);
    mLoadMoreHandler.setLoadFinishMsg(R.string.slotmachine_manage_deallist_load_finish_footer);
  ...
  private void getData(String url, final Map<String, String> bizParams) {
    ...
    String URL1 = url + "?mid="
            + Constants.userId + "&pageSize=20&pageNumber=" + mBizParams.get(PAGE_PARAM);
    StringRequest dataRequest = new StringRequest(Request.Method.GET,
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            Gson gson = new Gson();
            List<TradeBean> tradeBeans = gson.fromJson(result,
                    new TypeToken<List<TradeBean>>() {
                    }.getType());
            updateListData(tradeBeans);
          } catch (Exception e) {
          }
        }
      }
  ...
  private void getFirstData() {
    String URL1 = url + "?mid="
            + Constants.userId + "&pageSize="
            + 20 + "&pageNumber=" + mBizParams.get(PAGE_PARAM);
    StringRequest dataRequest = new StringRequest(Request.Method.GET,
            URL1, new Response.Listener<String>() {
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            ...
            if (tradeBeans != null && tradeBeans.size() < 20) {
              removeFooter();
            }
            if (mRequestListener != null){
              mRequestListener.onRequestSuccess(tradeBeans);
            }
            // 下拉刷新时，需要清空数据
