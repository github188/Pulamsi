 ...
 * <ul>
 * 改版履历:
 * <li> Created by daidingkang on 2016/3/20.
 * <li> 2016/4/11 内存泄露修正
 * <li>  2016/4/13 广告栏高度根据子控件进行自动调整。如果想设置广告栏高度不变，可以设置定长高度。
 * <li>  2016/5/4 动态刷新出现视图叠加和残留
 */
@SuppressLint("NewApi")
public class RollAdverView extends LinearLayout {
    ...
    public RollAdverView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public RollAdverView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.RollAdverView);
        mAdverHeight = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, jdAdverHeight, getResources().getDisplayMetrics());
        mGap = array.getInteger(R.styleable.RollAdverView_gap, GAP);
        mAnimDirection = array.getInteger(R.styleable.RollAdverView_animDirection, DIRECTION_UP);
        animDuration = array.getInteger(R.styleable.RollAdverView_animDuration, mAnimDuration);
        //关闭清空TypedArray
        array.recycle();
    }
     ...
     * 设置数据
     */
    public void setAdapter(RollViewAdapter adapter) {
