...
//        canvas.translate(20,30);偏移位置，防止遮挡
        ...
        for (int i = 0; i < nodeProgressAdapter.getCount(); i++) {
            ...
            if (i == 0) {
                ...
                //画文字
                mPaint.setTextSize(DensityUtil.sp2px(context, 15));
...
//                canvas.drawText("不换行", left * 2 + nodeRadius * 2 + 10, i * nodeInterval + top + (nodeInterval / 3), mPaint);
                ...
                StaticLayout layout = new StaticLayout(((LogisticsData)data.get(i)).getContext()+"", textPaint, (int)(canvas.getWidth() * 0.9), Layout.Alignment.ALIGN_NORMAL, 1.0F, 0.0F, false);
                canvas.save();
