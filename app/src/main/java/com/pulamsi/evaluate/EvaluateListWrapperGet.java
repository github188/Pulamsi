 ...
 *
 */
public class EvaluateListWrapperGet extends RecyclerView.OnScrollListener implements  PtrHandler {
  ...
  /** 列表对应数据适配器 */
  private HaiBaseListAdapter<Estimate> mListAdapter;
  ...
  public EvaluateListWrapperGet(Activity activity) {
    mActivity = activity;
  }

  public EvaluateListWrapperGet setListView(TRecyclerView listView) {
    mListView = listView;
    return this;
  }

  public EvaluateListWrapperGet setLayoutManager(StaggeredGridLayoutManager layoutManager) {
    mLayoutManager = layoutManager;
    return this;
  }

  public EvaluateListWrapperGet setListAdapter(HaiBaseListAdapter adapter) {
    mListAdapter = adapter;
    return this;
  }
  public EvaluateListWrapperGet setProgressWheel(ProgressWheel progressWheel) {
    this.progressWheel = progressWheel;
    return this;
  }

  public EvaluateListWrapperGet setLoadMoreHandler(CommonListLoadMoreHandler loadMoreHandler) {
    mLoadMoreHandler = loadMoreHandler;
    mLoadMoreHandler.setLoadingMsg(R.string.good_detail_evaluate_loading_more_footer);
    mLoadMoreHandler.setLoadFinishMsg(R.string.good_detail_evaluate_load_finish_footer);
    mIsLoadMoreOpen = true;
    return this;
  }

  public EvaluateListWrapperGet setPtrStylelFrameLayout(PtrStylelFrameLayout ptrStylelFrameLayout) {
    this.ptrStylelFrameLayout = ptrStylelFrameLayout;
    mIsPullToRefreshOpen = true;
    return this;
  }

  public EvaluateListWrapperGet setRequestListener(RequestListener requestListener) {
    mRequestListener = requestListener;
    return this;
  }

  public EvaluateListWrapperGet setOnScrollListener(OnScrollListener onScrollListener) {
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            ...
            updateListData(estimateList.getList());
          } catch (Exception e) {
          }
        }
      }
      ...
      public void onResponse(String result) {
        if (result != null) {
          try {
            Gson gson = new Gson();
            EstimateList  estimateList = gson.fromJson(result, EstimateList.class);
            if (estimateList.getList() != null && estimateList.getList().size() < 20) {
              removeFooter();
            }
            if (mRequestListener != null){
              mRequestListener.onRequestSuccess(estimateList.getList());
            }
            // 下拉刷新时，需要清空数据
